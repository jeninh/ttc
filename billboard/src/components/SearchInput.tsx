...
export default function SearchInput({ id, placeholder, icon, value, onChange, onSelect, accent }: Props) {
  ...
  const handleInput = (text: string) => {
    ...
    debounceRef.current = window.setTimeout(async () => {
      ...
      const filtered = addressResults.filter((a) => !stationLabels.has(a.label.toLowerCase()))
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      if (activeIdx >= 0 && activeIdx < results.length) {
        ...
        selectResult(results[0]) // Select first result if none focused
      }
  ...
  return (
    <div ref={wrapperRef} className="search-input-wrap" style={{ '--accent': accent } as any}>
      ...
      {showDropdown && results.length > 0 && (
        <ul ref={listRef} className="search-dropdown">
          {results.map((r, i) => (
            <li
              key={i}
              className={`search-option${r.isStation ? ' is-station' : ''}${i === activeIdx ? ' active' : ''}`}
              onClick={() => selectResult(r)}
              onMouseEnter={() => setActiveIdx(i)}
            >
              <span className="option-icon">{r.isStation ? '🚇' : '📍'}</span>
